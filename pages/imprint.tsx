...
import { As, useColorMode } from '@chakra-ui/system';
...
export const Imprint: NextPage = () => {
  ...
  const HiddenEmail: FunctionComponent = () => (
    <>
      {!isCaptchaVisible && !isMailVisible && <Button onClick={() => setIsCaptchaVisible(true)}>Show E-Mail</Button>}
      {isCaptchaVisible && !isMailVisible && (
        <ReCAPTCHA
          theme={colorMode}
      ...
      {isMailVisible && (
        <Link href="mailto:max@friedrich.dev" weight="bold">
          max@friedrich.dev
        </Link>
      )}
    </>
  ...
  return (
    <PageLayout>
      ...
      <HStack pb="10rem">
        <Text>Quelle:</Text>
        <Link href="https://www.e-recht24.de/impressum-generator.html" weight="bold">
